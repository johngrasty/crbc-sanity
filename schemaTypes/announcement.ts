...
export default defineType({
  ...
  fields: [
    defineField({
      ...
      validation: (Rule) =>
        Rule.required().max(100).warning('Keep titles concise for better display'),
      group: 'content',
    }),
    defineField({
      name: 'slug',
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.required().max(200).warning('Keep descriptions brief for carousel display'),
      group: 'content',
    }),
    defineField({
      name: 'image',
      ...
      fields: [
        {
          ...
          description:
            'Use AI Assist (✨) to generate, then review. 10 to 125 characters. Describe what is in the photo. Do not start with "image of".',
          validation: (Rule) =>
            Rule.required()
              .min(10)
              .max(125)
    ...
    defineField({
      ...
      type: 'url',
      description:
        'URL for the call-to-action button (can be relative like /calendar or absolute like https://...)',
      validation: (Rule) =>
        Rule.uri({
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.min(Rule.valueOfField('startDate')).error('End date must be after start date'),
      group: 'scheduling',
    }),
  ],
  orderings: [
